...
static int send_next_capture_conf_to_fw(struct a1000_isp_device *isp);
...
static int isp_firmware_init(struct a1000_isp_device *isp)
{
	...
	cmdp_size = (sizeof(tSoneCmdp) + SONE_PART_ALIGN_MASK) &
		    (~SONE_PART_ALIGN_MASK);
	pr_debug("cmdp start = 0x%llx, cmdp_size = 0x%lx, align size = 0x%x\n",
	...
	pr_debug("slab start = 0x%llx, slab_size = 0x%x\n", slab_phy_base,
		 SONE_MEDIA_SLAB_BUFSIZE);
	...
	pr_debug("===== partition_total_size = 0x%x\n", partition_total_size);
	...
	pr_debug(
		"isp driver: partition_vaddr = 0x%x, , dma_addr = 0x%llx, size = 0x%x\n",
		*partition_vaddr, partition_dma_addr, partition_total_size);
...
static void isp_cleanup_videos(struct a1000_isp_device *isp)
{
	...
	for (i = 0; i < MAX_ISP_CHANNEL; i++) {
		...
		a1000_isp_video_cleanup(&isp->channels[i].pdns_video);
	}
}
...
static int channel_capture_conf_list[MAX_ISP_CHANNEL] = { -1 };
...
static inline int is_channel_in_capture_mode(struct a1000_isp_device *isp,
					     int index)
{
	return (isp->channels[index].cam_dev->yuv_cap_enabled ||
		isp->channels[index].cam_dev->raw_cap_enabled);
}
...
static inline int is_channel_capture_conf_left(struct a1000_isp_device *isp)
{
	pr_debug("%s, capture_conf_num %d, capture_conf_count %d\n", __func__,
		 isp->capture_conf_num, isp->capture_conf_count);
	return (isp->capture_conf_count < isp->capture_conf_num);
}
...
static int next_channel_capture_conf_index(struct a1000_isp_device *isp)
{
	int index;
	int i;

	if (isp->capture_conf_count >= isp->capture_conf_num)
		return -1;

	if (channel_capture_conf_list[0] < 0) {
		index = 0;
		for (i = 0; i < MAX_ISP_CHANNEL; i++) {
			if (isp->channels[i].enable &&
			    is_channel_in_capture_mode(isp, i)) {
				channel_capture_conf_list[index] = i;
				index++;
			}
		}
	}

	return channel_capture_conf_list[isp->capture_conf_count];
}
...
static int ispdrv_enter_capture_conf_state(struct a1000_isp_device *isp)
{
	send_next_capture_conf_to_fw(isp);
	isp->state = ISPDRV_STATE_SET_CAPTURE_CONF;
	return 0;
}
				...
				struct media_command *msg, int64_t ktime)
{
	...
	if ((msg->cmd_hdr.src[0] == 'i') && (msg->cmd_hdr.src[1] == 'i')) {
		...
		copy_msg_to_video_cache(
			...
			cmd_main, cmd_minor, *tick, ktime, msg->user_cmd_data,
			4);
	...
	if ((msg->cmd_hdr.src[0] == 'p') && (msg->cmd_hdr.src[1] == 'd')) {
		...
		isp->kthread_status = KTHREAD_STATUS_COPY_MSG;
		copy_msg_to_video_cache(
			&(isp->channels[isp_entity_index].pdns_video), cmd_main,
				  ...
				  int64_t ipc_timestamp)
{
	...
	case ISPDRV_STATE_SCFG:
		if (message == MINOR_ISP_BOOTLD_RECONF) {
			...
			else if (is_channel_capture_conf_left(isp))
				ispdrv_enter_capture_conf_state(isp);
			else
				ispdrv_enter_start_state(isp);
		} else {
			ispdrv_state_error(isp, message);
	...
	case ISPDRV_STATE_SET_DSP_PWL:
		...
		break;
	case ISPDRV_STATE_SET_CAPTURE_CONF:
		if (message == MINOR_ISP_CAPTURE_RECONF) {
			isp_capture_reconf_t *capture_reconf_cmd;

			capture_reconf_cmd = (isp_capture_reconf_t *)&(
				cmd->user_cmd_data[0]);
			pr_info("ISPDRV_STATE_SET_CAPTURE_CONF, MINOR_ISP_CAPTURE_RECONF\n");
			pr_info("sensorIndex: %u, captureMode: 0x%08X, totalRawSize: %u\n",
				capture_reconf_cmd->sensorIndex,
				capture_reconf_cmd->captureMode,
				capture_reconf_cmd->totalRawSize);
			isp->channels[capture_reconf_cmd->sensorIndex]
				.views_video.cap_raw_size =
				capture_reconf_cmd->totalRawSize;

			if (is_channel_capture_conf_left(isp))
				send_next_capture_conf_to_fw(isp);
			else
				ispdrv_enter_start_state(isp);
		} else {
...
static int isp_kthread_recv_msg(void *data)
{
	...
	while (1) {
		...
		media_msg = (struct media_command *)(isp->cmdp_vaddr +
						     msg_addr_offset);
		// pr_info("msg_phy_addr: 0x%llx\n",msg_phy_addr);
		// pr_info("msg_addr_offset: 0x%llx\n",msg_addr_offset);
		// pr_info("media_msg addr: 0x%llx\n",media_msg);
...
static int send_capture_msg_to_fw(struct a1000_isp_device *isp, int index)
{
	u32 media_cmd_paddr;
	struct media_command *media_cmd;
	isp_capture_reconf_t *capture_reconf_cmd;
	...
	pr_info("start media_cmd_paddr = 0x%x\n", media_cmd_paddr);
	memset(media_cmd, 0, sizeof(struct media_command));
	media_cmd->cmd_hdr.hdr_info.cmd_type_minor = MINOR_ISP_CAPTURE_RECONF;
	capture_reconf_cmd =
		(isp_capture_reconf_t *)(&media_cmd->user_cmd_data[0]);
	capture_reconf_cmd->sensorIndex = index;
	if (isp->channels[index].cam_dev->yuv_cap_enabled) {
		capture_reconf_cmd->capModeBits.yuvCapEn = 1;
		capture_reconf_cmd->capModeBits.drvCapEn = 1;
	}
	if (isp->channels[index].cam_dev->raw_cap_enabled) {
		capture_reconf_cmd->capModeBits.rawCapEn = 1;
		capture_reconf_cmd->capModeBits.drvCapEn = 1;
	}
			     ...
			     struct camera_dev *cam_dev, int chn_index)
{
	...
	reconf = (ipc_reconf_t *)payload_vaddr;
	if (cam_dev->raw_cap_enabled) {
		/*
		 * TODO:
		 *
		 * In capture mode, view 2 should be closed, but we must tell
		 * app view 0~2 are available, this should be covered by ISP
		 * firmware.
		 */
		pr_info("%s: in capture mode, chn %d's view 2 will be disabled\n",
			__func__, chn_index);
		reconf->viewinfo[2].viewFmt = View2_Dis;
	}
	memset(media_cmd, 0, sizeof(struct media_command));
			       ...
			       const char *filename)
{
	...
	pr_debug("=== send algo bin sensorIndex = %d\n", chn_index);
...
static int send_next_capture_conf_to_fw(struct a1000_isp_device *isp)
{
	int index = next_channel_capture_conf_index(isp);

	if (index < 0)
		return 0;

	send_capture_msg_to_fw(isp, index);
	isp->capture_conf_count++;
...
static int init_isp_dev(struct a1000_isp_device *isp_dev)
{
	...
	for (i = 0; i < MAX_MIPI_DEVICE_NUM; i++) {
		...
		pr_debug("asd_index %d, fwnode = 0x%p\n", asd_index,
				       ...
				       int chn_id)
{
	...
	// init pdns video
	video = &(isp_channel->pdns_video);
	isp_channel_init_pdns_video(isp_dev, isp_channel, chn_id);
	ret = a1000_isp_video_register(video, &isp_dev->v4l2_dev);
	if (ret < 0) {
		dev_err(isp_dev->dev, "%s register pdns video failed\n",
			__func__);
		return -1;
	}
	// must after video register
	...
	for (i = 0; i < MAX_ISP_CHANNEL; i++) {
		...
		if (isp_chn->enable) {
			if ((isp_chn->csi_channel) &&
				...
				// pr_debug("isp_chn i = %d, cam_dev = 0x%lx\n",
				// i, isp_chn->cam_dev);
				...
				if (is_channel_use_pwl_lut(isp, i))
					isp->pwl_lut_num++;
				if (is_channel_in_capture_mode(isp, i))
					isp->capture_conf_num++;
			} else {
