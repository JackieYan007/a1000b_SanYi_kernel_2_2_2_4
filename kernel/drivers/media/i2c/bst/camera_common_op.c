				 ...
				 struct device_node *node)
{
	...
	if (ret == 0)
		...
		cam_dev->isp_data.linesAddDetBad = 0;
	...
	cam_dev->yuv_cap_enabled = of_property_read_bool(node, "cap-yuv");
	cam_dev->raw_cap_enabled = of_property_read_bool(node, "cap-raw");
	ret = of_property_read_u32(node, "cap-buf-shift", &value);
	if (ret == 0)
		cam_dev->cap_buf_shift = value;
